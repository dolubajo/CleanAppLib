...
namespace AppLib.Collections
{
  public static class Extensions
  {
    ...
    public static IEnumerable<IPositionedElement> PrepareRemoval(this IEnumerable<IPositionedElement> index, int removeAt)
    {
      Guard.IsNotNull(index, "index");
      Guard.IsNotNull(removeAt, "remove_at");
      Guard.EnforcePredicate(removeAt >= 1, "remove_at.postion out of range. An index starts at 1.");
      Guard.EnforcePredicate(removeAt <= index.Count(), "remove_at.postion out of range. Can not insert an element which would result in the index not being contiguous.");
    ...
    public static IEnumerable<IPositionedElement> PrepareRelocation(this IEnumerable<IPositionedElement> index, int from, int to)
    {
      ...
      Guard.EnforcePredicate(to <= index.Count(), "To Out of range. Can not insert an element which would result in the index not being contiguous.");
